...
def is_valid_relation(pair, category):
    ...
    # 推荐药物
        if 'B-DIS' in find_entity(pair[0]).values() \
        and 'B-DRU' in find_entity(pair[1]).values():
            return True
        if 'B-DRU' in find_entity(pair[0]).values() \
        and 'B-DIS' in find_entity(pair[1]).values():
            return True
    if category == 'cause':
    # 引起
        if ('B-BAC' or 'B-DIS' or 'B-VIR') in find_entity(pair[0]).values() \
        and ('B-DIS' or 'B-SYM') in find_entity(pair[1]).values():
            return True
    # TODO: 相似疾病症状
    if category == 'detect':
    # 检测
        if 'B-INS' in find_entity(pair[0]).values() \
        and ('B-DRU' or 'B-DIS' or 'B-SYM' or 'B-VIR') in find_entity(pair[1]).values():
            return True
    if category == 'disease':
    # 病症
        if ('B-DIS' or 'B-SYM') in find_entity(pair[0]).values() \
        and 'B-SYM' in find_entity(pair[1]).values():
            return True
    if category == 'inspection':
    # 检查
        if 'B-SPE' in find_entity(pair[0]).values() \
        and (('B-DIS' or 'B-SYM') in find_entity(pair[1]).values()):
            return True
